...
function publishDevlog() {
  ...
  p.projectTrust = clamp(p.projectTrust + randomInt(-2, 4), 0, 100);
...
function interactCommunity() {
  ...
  if (Math.random() < 0.12) {
    ...
    p.projectTrust = clamp(p.projectTrust + randomInt(2, 8), 0, 100);
...
function hireInfluencer(size) {
  const p = gameState.currentProject;
  const options = {
    small: { label: "pequeno", cost: 3000, hype: [5, 12], trust: [-2, 4], score: 8 },
    medium: { label: "medio", cost: 10000, hype: [12, 25], trust: [-5, 8], score: 16 },
    large: { label: "grande", cost: 25000, hype: [25, 45], trust: [-10, 12], score: 28 }
  };
  ...
  let trustGain = randomInt(option.trust[0], option.trust[1]);
  if (p.progress.polish < 40) trustGain -= randomInt(4, 12);
  if (p.progress.polish > 75) trustGain += randomInt(3, 8);
  p.projectTrust = clamp(p.projectTrust + trustGain, 0, 100);
...
function launchBeta() {
  ...
  const coverage = averageCoverage(p);
  let text = "";

  if (polish < 30) {
    p.hype = clamp(p.hype - randomInt(10, 25), 0, 100);
    p.projectTrust = clamp(p.projectTrust - randomInt(15, 30), 0, 100);
    p.betaResult = -20;
    ...
    p.projectTrust = clamp(p.projectTrust + (mixed ? -10 : -5) + Math.round(coverage), 0, 100);
    p.betaResult = mixed ? -4 : -8;
    text = "O beta chamou atencao, mas revelou problemas em alta definicao emocional.";
  } else if (polish < 80) {
    p.hype = clamp(p.hype + randomInt(10, 25), 0, 100);
    p.projectTrust = clamp(p.projectTrust + randomInt(5, 15), 0, 100);
    p.betaResult = 12;
    ...
    p.projectTrust = clamp(p.projectTrust + randomInt(10, 25), 0, 100);
    p.betaResult = 22;
...
function delayGame(weeks) {
  ...
  if (p.delays === 1) {
    p.projectTrust = clamp(p.projectTrust - 10, 0, 100);
    p.hype = clamp(p.hype - 5, 0, 100);
  } else if (p.delays === 2) {
    p.projectTrust = clamp(p.projectTrust - 25, 0, 100);
    p.hype = clamp(p.hype - 10, 0, 100);
  } else {
    p.projectTrust = clamp(p.projectTrust - 50, 0, 100);
