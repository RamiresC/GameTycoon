...
function renderCompanyDetails(companies) {
  return companies.map((company, index) => {
    const last = company.lastGame;
    const next = Math.max(0, company.nextReleaseWeek - gameState.week);
    const lastText = last
      ? `${last.name}, nota ${last.review}, ${prettyNumber(last.totalSales || last.sales)} copias`
      : "Nenhum lancamento recente";

    return `
      <p>
        ${index + 1}. ${company.name} - ${prettyNumber(company.score)} pts
        <br><span class="subtle">Proximo lancamento: ${next} semanas</span>
        <br><span class="subtle">Ultimo jogo: ${lastText}</span>
      </p>
    `;
  }).join("");
}
...
function renderRankings() {
  ...
  const bySales = [...gameState.releasedGames].sort((a, b) => b.totalSales - a.totalSales).slice(0, 5);
  const companies = getCompanyRanking().slice(0, 5);
  const studioPosition = getStudioMarketPosition();
  ...
  return `
    <section class="panel">
      ...
      <div class="ranking-grid">
        <div class="card">
          <h3>Top jogos por vendas</h3>
          ${bySales.length ? bySales.map((game, index) => `<p>${index + 1}. ${game.name} - ${prettyNumber(game.totalSales)} copias</p>`).join("") : "<p class='subtle'>Nenhum jogo lancado ainda.</p>"}
        </div>
        <div class="card">
          <h3>Top 5 empresas</h3>
          ${renderCompanyDetails(companies)}
          <p class="subtle">Sua posicao: ${studioPosition.position}/${studioPosition.total}</p>
          <p class="subtle">Pontuacao indie: ${prettyNumber(studioPosition.score)} pts</p>
        </div>
      </div>
    </section>
  `;
