// Arquivo: js/systems/week.js
// Responsabilidade: Avanco semanal, contas, focos de desenvolvimento e eventos aleatorios.

// 5. Semana
/** Avanca o calendario e processa mercado, vendas, contas e prazos. */
function advanceWeek(options = {}) {
  ...
  const reachedDeadline = project && gameState.week >= project.releaseWeek;
  if (project && !reachedDeadline && options.randomEvent !== false) {
    triggerRandomEvent();
  }
...
/** Avanca uma semana por meio da cama. */
function sleepOneWeek() {
  if (typeof handleTutorialSleepAttempt === "function" && handleTutorialSleepAttempt()) return;

  addLog("Voce descansou uma semana. O calendario avancou sem pedir licenca.");
  advanceWeek({ randomEvent: false });
}

/** Processa monthly bills. */
...
/** Aplica development foco. */
function applyDevelopmentFocus(area) {
  if (typeof handleTutorialDevelopmentFocusAttempt === "function" && handleTutorialDevelopmentFocusAttempt(area)) return;
